...
const AdminStylists = () => {
  ...
  const [stylists, setStylists] = useState<Stylist[]>([
    {
      ...
      reviews: [
        {
          rating: 5,
          text: "Amazing work with my wedding makeup!",
          author: "Sara H.",
        },
    ...
    },
    {
      ...
      reviews: [
        {
          rating: 4.5,
          text: "Great service, will book again",
          author: "Mariam F.",
        },
      ],
    },
  ...
  const handleAddStylist = (newStylist: {
    ...
    const newId =
      Math.max(...[...stylists, ...pendingStylists].map((s) => s.id)) + 1;
  ...
  const handleStatusToggle = (stylist: Stylist) => {
    const updatedStylists = stylists.map((s) => {
      if (s.id === stylist.id) {
        return {
          ...s,
          status:
            s.status === "active" ? ("offline" as const) : ("active" as const),
        };
      }
      return s;
    });
    ...
    setStylists(updatedStylists);
    toast.success(
      `${stylist.name} is now ${stylist.status === "active" ? "offline" : "online"}`,
    );
  };
