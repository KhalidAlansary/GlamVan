...
const BeauticianSelection = ({ 
  ...
  const [selectedBeautician, setSelectedBeautician] = useState(bookingData.beautician);
  ...
  const filteredBeauticians = beauticians.filter(beautician => {
    ...
    if (categories.includes("wedding") && !beautician.specialties.includes("wedding")) {
      return false;
    }
    
    // Check if beautician has at least one of the required specialties
    return categories.some(category => beautician.specialties.includes(category));
  });
    ...
    <div className="space-y-6">
      ...
      {filteredBeauticians.length === 0 ? (
        <div className="text-center p-6 border border-dashed border-gray-300 rounded-md">
          <p className="text-gray-500">No beauticians available for these services. Please try different services or contact us directly.</p>
        </div>
      ) : (
        <RadioGroup
          ...
          {filteredBeauticians.map((beautician) => (
            <div
              ...
              className={`border rounded-md p-4 cursor-pointer hover:border-salon-purple transition-all ${
                selectedBeautician === beautician.id ? "border-salon-purple bg-salon-purple/5" : "border-gray-200"
              }`}
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  >
                    ...
                    <span className="text-gray-500 text-sm">{beautician.experience}</span>
                  </Label>
