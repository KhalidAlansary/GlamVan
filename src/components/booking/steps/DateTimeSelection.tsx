...
import { format, addDays, addMonths, isBefore, isPast, startOfDay, isToday } from "date-fns";
    ...
    <div className="space-y-6">
      ...
      {isWedding ? (
        <div className="p-4 border-l-4 border-salon-pink bg-salon-pink/10 text-sm mb-4">
          ...
          <p className="text-red-600 font-medium">Important: Wedding appointments must be booked at least 1 month in advance.</p>
          <p className="mt-2">This includes a trial session (scheduled 2 weeks before) and our premium bridal services package.</p>
        </div>
      ) : (
        <div className="p-4 border-l-4 border-salon-purple/70 bg-salon-purple/5 mb-4">
          ...
          <p className="text-red-600 font-medium">All beauty services must be booked at least 1 week in advance.</p>
          ...
          <Calendar
            ...
            onSelect={(date) => {
              if (date) {
                ...
                if (!isWedding && isBefore(date, recommendedDate) && !isPast(date)) {
                  toast.info("We recommend booking at least 1 week in advance for the best availability");
                }
